...
public class ProfileController {
    ...
    public ProfileController(PatientService patientService, DoctorService doctorService, ClinicService clinicService, JwtUtil jwtUtil) {
    ...
    public ResponseEntity<?> getProfile(
        ...
        return switch (role) {
            case "PATIENT" -> buildResponse(patientService.findByUserId(userId));
            case "DOCTOR" -> buildResponse(doctorService.findByUserId(userId));
            case "CLINIC" -> buildResponse(clinicService.findByUserId(userId));
            default -> ResponseEntity.badRequest().body("Không tồn tại role");
        };
    }

    private <T> ResponseEntity<?> buildResponse(Optional<T> profileOpt) {
        return profileOpt
